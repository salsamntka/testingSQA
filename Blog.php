<?php
// controller ini bertanggung jawab untuk menampilkan halaman blog 
...
class Blog extends CI_Controller{
	...
	// dilakukan proses untuk menampilkan halaman utama blog
	// Prosesnya termasuk mengambil jumlah total berita, mengatur pagination, dan menampilkan data berita per halaman
	function index(){
		// berita() terdapat dalam m_tulisan untuk select seluruh berita
        ...
        $config['base_url'] = base_url() . 'blog/index/';
            ...
            $x['page'] =$this->pagination->create_links();
						// berita_perpage() terdapat dalam m_tulisan untuk select seluruh berita perhalaman berdasarkan offset dan limit
						$x['data']=$this->m_tulisan->berita_perpage($offset,$limit);
						$x['category']=$this->db->get('tbl_kategori');
						$x['populer']=$this->db->query("SELECT * FROM tbl_tulisan WHERE status=1 ORDER BY tulisan_views DESC LIMIT 5");
						$this->load->view('depan/v_blog',$x);
	}

	// digunakan untuk menampilkan halaman detail dari sebuah tulisan di blog
    // Prosesnya termasuk mengambil data tulisan berdasarkan slug yang diberikan, mengupdate jumlah views, dan menampilkan informasi tulisan beserta komentar yang terkait
	...
	// digunakan untuk menampilkan tulisan berdasarkan kategori tertentu. 
	// Prosesnya termasuk mengambil data tulisan berdasarkan kategori yang dipilih dan menampilkan hasilnya
	function kategori(){
	...
	// digunakan untuk melakukan pencarian tulisan berdasarkan kata kunci yang diberikan. 
	// Prosesnya termasuk mencari data tulisan yang sesuai dengan kata kunci dan menampilkan hasilnya
    function search(){
		...
		// digunakan untuk menyimpan komentar yang diberikan oleh pengguna pada sebuah tulisan
		// Prosesnya termasuk validasi input komentar dan menyimpan komentar ke dalam database
		function komentar(){
			// htmlspecialchars yaitu mengkonversi beberapa karakter khusus menjadi entitas HTML
